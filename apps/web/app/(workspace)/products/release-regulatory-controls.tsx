...
export function ReleaseRegulatoryControls({
  ...
  enabled: boolean;
  onReload: () => void;
  section?: "all" | "lifecycle" | "support";
}) {
  const showLifecycle = section !== "support";
  const showSupport = section !== "lifecycle";
  const memberStates = useMemberStatesQuery(enabled && showLifecycle);
                ...
                >
                  <label className="text-caption-1-regular text-fg">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setTargetState(
                          event.target.value as ReleaseLifecycleState,
                        )
                      }
                      className="mt-1 h-10 w-full rounded-xl border border-border bg-canvas px-3 text-caption-1-regular text-fg"
                    >
