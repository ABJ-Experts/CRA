...
import type { Coin, Customer, Order, Product } from "./table-schemas";
...
const SEED_PRODUCTS: Omit<Product, "id">[] = [
  ...
  },
  {
    no: 3,
    sku: "UG1234123",
    name: "Heimer Miller Sofa (Mint Condition)",
    updatedAt: "21 Aug 2022",
    category: "Sports Outlet",
    status: "Active",
    quantity: 192,
    revenue: "$11,457",
    price: "$767.50",
...
const SEED_ORDERS: Omit<Order, "id">[] = [
  {
    no: 1,
    orderNo: "0000982551",
    trackingId: "1MwvM5j6J1bkvry",
    createdAt: "28 Nov 1988, 00:38",
    source: "Gloves",
    status: "Opened",
    quantity: 287,
    price: "$928.41",
  },
  {
    no: 2,
    orderNo: "0000982553",
    trackingId: "BrQv91mWzywzmvzg",
    createdAt: "25 Jan 2002, 07:10",
    source: "Activewear",
    status: "Closed",
    quantity: 59,
    price: "$328.85",
  },
  {
    ...
    orderNo: "0000982542",
    trackingId: "BrQv91mWzywzmvzg",
    createdAt: "17 May 1996, 19:11",
    source: "Sports Outlet",
    status: "Delivered",
    quantity: 192,
    price: "$767.50",
  },
...
export const PRODUCTS: Product[] = (() => {
  ...
  const out: Product[] = SEED_PRODUCTS.map((p, i) => ({
    ...p,
    id: rid("prd", i),
  }));
  ...
  for (let i = SEED_ORDERS.length; i < 120; i++) {
    out.push({
      ...
      trackingId: Array.from(
        { length: 15 },
        () =>
          "abcdefghijkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ0123456789"[
            intBetween(rand, 0, 57)
          ],
      ).join(""),
...
export const CUSTOMERS: Customer[] = (() => {
  ...
  const out: Customer[] = SEED_CUSTOMERS.map((c, i) => ({
    ...c,
    id: rid("cus", i),
  }));
...
export const COINS: Coin[] = (() => {
  ...
  for (let i = SEED_COINS.length; i < 60; i++) {
    const [name, symbol] = COINS_MORE[
      (i - SEED_COINS.length) % COINS_MORE.length
    ...
    out.push({
      ...
      name:
        i < SEED_COINS.length + COINS_MORE.length
          ? name
          : `${name} ${Math.floor(i / COINS_MORE.length)}`,
      symbol,
