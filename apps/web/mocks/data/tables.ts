  ...
  for (let i = SEED_ORDERS.length; i < 120; i++) {
    out.push({
      ...
      trackingId: Array.from(
        { length: 15 },
        () => "abcdefghijkmnpqrstuvwxyzABCDEFGHJKLMNPQRSTUVWXYZ0123456789"[intBetween(rand, 0, 57)],
      ).join(""),
...
export const COINS: Coin[] = (() => {
  ...
  for (let i = SEED_COINS.length; i < 60; i++) {
    const [name, symbol] = COINS_MORE[(i - SEED_COINS.length) % COINS_MORE.length] as readonly [
      string,
      string,
    ];
