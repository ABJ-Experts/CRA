...
select ok((select tgname='technical_file_declaration_immutable' from pg_trigger where tgrelid='public.technical_file_declarations'::regclass and not tgisinternal), 'issued declaration immutability trigger exists');
...
select throws_ok($$insert into public.technical_file_declarations(organization_id,product_id,snapshot_id,template_id,declaration_version,signatory_user_id,signatory_name,signatory_capacity,issue_place,assessment_route,notified_body_identifier,certificate_references,idempotency_key,command_digest) values(gen_random_uuid(),gen_random_uuid(),gen_random_uuid(),(select id from public.technical_file_declaration_templates limit 1),1,gen_random_uuid(),'Signer','Officer','Brussels','eu_type_examination','1234','[]'::jsonb,gen_random_uuid(),repeat('a',64))$$, '23503', NULL, 'cross-tenant invalid references cannot be inserted');
