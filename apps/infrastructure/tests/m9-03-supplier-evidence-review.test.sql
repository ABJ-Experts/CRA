...
select ok(position('review_row.decision = ''accepted''' in pg_get_functiondef('public.m9_03_review_item_json_before_m906(uuid,uuid)'::regprocedure)) > 0
  and position('m9_03_review_item_json_before_m906' in pg_get_functiondef('public.m9_03_review_item_json(uuid,uuid)'::regprocedure)) > 0, 'per-item state keeps acceptance distinct from scan state');
