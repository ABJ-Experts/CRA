...
select ok(position('m9_02_current_draft' in pg_get_functiondef('public.m9_02_issue_invitation(uuid,uuid,uuid,integer,text,text,timestamp with time zone,uuid,text)'::regprocedure))>0, 'issue derives its preview fingerprint from the canonical draft');
...
select ok(position('state=''revoked''' in pg_get_functiondef('public.m9_02_issue_invitation(uuid,uuid,uuid,integer,text,text,timestamp with time zone,uuid,text)'::regprocedure))>0, 'reissue revokes prior active sessions before creating a new invitation');
