...
select ok(position('can_share_technical_files' in pg_get_functiondef('public.create_technical_file_auditor_snapshot_grant_atomic(uuid,uuid,uuid,uuid,uuid,uuid,text,text,text,timestamptz,text,uuid,text)'::regprocedure))>0, 'grant creation checks its dedicated share permission');
