...
begin
 ...
 perform pg_temp.check('M6-03 approval preserves the separate filing boundary',
  (select state in ('running','overdue') and submitted_at is null from public.reporting_obligation_stages where organization_id=v_org and id=v_stage)
  and (select version=1 from public.reporting_obligations where organization_id=v_org and id=v_obligation)
