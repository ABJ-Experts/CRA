...
begin
  ...
  update public.finding_propagation_jobs j set
   ...
   where j.organization_id=p_organization_id and j.id=p_job_id returning j.checkpoint_version into v_job.checkpoint_version;
...
create or replace function public.list_due_finding_propagation_job_organizations()
returns table(organization_id uuid)
language sql security definer set search_path = public, pg_temp as $$
  select distinct j.organization_id
    from public.finding_propagation_jobs j
   where (j.status in ('scheduled','retrying') and j.due_at <= clock_timestamp())
...
alter function public.list_due_finding_propagation_job_organizations() owner to postgres;
revoke all on function public.persist_finding_propagation_page_atomic(uuid,uuid,uuid,integer,jsonb,text,boolean),public.fail_finding_propagation_job_atomic(uuid,uuid,uuid,integer,text,boolean),public.list_due_finding_propagation_job_organizations() from public,anon,authenticated;
grant execute on function public.persist_finding_propagation_page_atomic(uuid,uuid,uuid,integer,jsonb,text,boolean),public.fail_finding_propagation_job_atomic(uuid,uuid,uuid,integer,text,boolean),public.list_due_finding_propagation_job_organizations() to service_role;

-- A graph event may be product-wide. It still carries a product ID, then this
-- finding-owned selection expands only indexed source records in that tenant.
drop function if exists public.enqueue_finding_propagation_jobs_atomic(uuid,text,integer,uuid,uuid,timestamptz);
create function public.enqueue_finding_propagation_jobs_atomic(
  p_organization_id uuid,p_trigger_key text,p_graph_version integer,p_source_product_id uuid,
...
begin
  if p_organization_id is null or p_source_product_id is null or p_graph_version is null
     or char_length(btrim(coalesce(p_trigger_key,''))) not between 1 and 240
     or (p_source_release_id is not null and p_source_baseline_revision_id is not null) then
    return query select 'invalid_request'::text,0;return;
  end if;
  insert into public.finding_propagation_jobs(organization_id,source_finding_id,trigger_key,graph_version,source_release_id,source_baseline_revision_id,rule_version,as_of,requested_by)
  select s.organization_id,s.id,btrim(p_trigger_key)||':'||s.id::text,p_graph_version,s.source_release_id,s.source_baseline_revision_id,s.rule_version,coalesce(p_as_of,clock_timestamp()),s.updated_by
    from public.finding_propagation_sources s
   where s.organization_id=p_organization_id and s.status='active' and s.source_product_id=p_source_product_id
     and (p_source_release_id is null or s.source_release_id=p_source_release_id)
     and (p_source_baseline_revision_id is null or s.source_baseline_revision_id=p_source_baseline_revision_id)
  on conflict (organization_id,trigger_key) do nothing;
  get diagnostics v_count=row_count;
  return query select 'enqueued'::text,v_count;
end;
$$;
alter function public.enqueue_finding_propagation_jobs_atomic(uuid,text,integer,uuid,uuid,uuid,timestamptz) owner to postgres;
revoke all on function public.enqueue_finding_propagation_jobs_atomic(uuid,text,integer,uuid,uuid,uuid,timestamptz) from public,anon,authenticated;
grant execute on function public.enqueue_finding_propagation_jobs_atomic(uuid,text,integer,uuid,uuid,uuid,timestamptz) to service_role;
