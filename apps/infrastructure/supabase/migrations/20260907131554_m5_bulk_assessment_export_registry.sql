-- Bulk operation snapshots and attributable outcomes are retained tenant
-- evidence, so extend the existing VEX export source rather than add a new
-- export workflow. Command rows remain excluded as request-security material.
insert into public.organization_export_source_tables (
  ...
  ('vulnerability_vex_assessments', 'vulnerability_finding_assessment_bulk_operations', 'organization_id', 'id', 5),
  ('vulnerability_vex_assessments', 'vulnerability_finding_assessment_bulk_operation_targets', 'organization_id', 'id', 6)
...
-- Keep the snapshot lock set in sync with the newly registered immutable
-- sources, so an export cannot split an operation header from its outcomes.
...
language plpgsql security definer set search_path=public,pg_temp as $m5_bulk_export$
