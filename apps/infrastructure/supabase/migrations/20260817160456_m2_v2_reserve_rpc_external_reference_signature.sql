drop function if exists public.reserve_product_security_update_artifact_atomic(
  uuid, uuid, uuid, uuid, text, text, text, text, jsonb, text, text, text,
  bigint, text, timestamptz, uuid, uuid
);
...
begin
  ...
  if p_distribution_kind = 'external_reference'
     and jsonb_array_length(p_validated_external_references) = 0 then
    return query select 'invalid_request'::text, null::jsonb;
    return;
  end if;
