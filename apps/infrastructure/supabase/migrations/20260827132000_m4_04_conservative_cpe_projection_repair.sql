-- Keep CPE projection conservative in already-migrated local databases. The
-- previous repair projected missing worker proof as version-specific when the
-- component CPE had a version; missing proof must stay broad-family.
