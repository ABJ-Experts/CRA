-- Adds the 7 connector-sync tables (connector_secrets excluded -- ciphertext
-- credential material, not portable tenant record data) registered in
-- 20260820100000_m2_v2_connector_sync_foundation.sql to the snapshot
-- materializer's lock set. Same signature as the existing function, so this
-- is a body-only CREATE OR REPLACE, not a new migration of the table catalogue.
