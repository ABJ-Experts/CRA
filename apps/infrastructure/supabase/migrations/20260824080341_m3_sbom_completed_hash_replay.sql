...
begin
  ...
  if found then
    ...
    if v_document.state = 'completed' then
      ...
      return query select 'replayed'::text, public.sbom_document_json(p_organization_id, v_document.id); return;
    end if;
    ...
    return query select 'resumed'::text, public.sbom_document_json(p_organization_id, v_document.id); return;
  end if;
