-- Reachability evidence and source-review transitions are durable tenant audit
-- facts. The values are already sanitized at the M4-07 boundary; register
-- them as a single deterministic export source rather than dropping history.

insert into public.organization_export_sources (source_id, enabled, sort_order)
values ('vulnerability_reachability_review', true, 45)
...
insert into public.organization_export_source_tables (
  ...
  ('vulnerability_reachability_review', 'vulnerability_reachability_results', 'organization_id', 'id', 1),
  ('vulnerability_reachability_review', 'vulnerability_finding_review_events', 'organization_id', 'id', 2)
...
-- The materializer holds a share lock across every registered tenant source.
-- Reuse its established definition with the two M4-07 ledgers included so
-- an archive cannot split a review event from the evidence it references.
