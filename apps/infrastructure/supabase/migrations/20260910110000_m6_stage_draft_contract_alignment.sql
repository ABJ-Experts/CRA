-- Align deployments that already received the initial M6-03 schema with the
-- strict, shared wire contracts. This is intentionally additive/replacement-only.
...
returns jsonb language sql stable security definer set search_path = public, pg_temp as $$
 select jsonb_build_object(
  'id',t.id,'organizationId',t.organization_id,'obligationType',t.obligation_type,'stage',t.stage_kind,'name',t.name,'description',t.description,
...
declare d public.reporting_stage_drafts%rowtype; v_digest text; v_existing record; v_result jsonb; v_provenance jsonb; v_states jsonb; v_recorded_at text:=public.m6_utc_second_z(clock_timestamp()); begin
 ...
 select coalesce(jsonb_agg(state->>'countryCode' order by state->>'countryCode'),'[]'::jsonb) into v_states from jsonb_array_elements(p_member_states) state;
 ...
 select * into d from public.reporting_stage_drafts where organization_id=p_organization_id and id=p_draft_id for update; if not found then return query select 'not_found',null::jsonb;return;end if;
 if d.version<>p_expected_version then return query select 'conflict',jsonb_build_object('draft',public.m6_reporting_draft_json(p_organization_id,d.id));return;end if;
 if d.lock_expires_at>clock_timestamp() and (d.locked_by_user_id<>p_actor_user_id or d.lock_token<>p_lock_token) then return query select 'locked',jsonb_build_object('draft',public.m6_reporting_draft_json(p_organization_id,d.id));return;end if;
...
declare d public.reporting_stage_drafts%rowtype; t public.reporting_family_templates%rowtype; v jsonb; v_content jsonb; v_provenance jsonb; v_template_id uuid; v_template_version integer; v_digest text; v_existing record; v_result jsonb; begin
 ...
 if found then return query select v_existing.outcome,v_existing.result; return; end if;
 select * into d from public.reporting_stage_drafts where organization_id=p_organization_id and id=p_draft_id for update; select v.template_id,v.version,v.content into v_template_id,v_template_version,v from public.reporting_family_template_versions v join public.reporting_family_templates t0 on t0.organization_id=v.organization_id and t0.id=v.template_id and t0.archived_at is null where v.organization_id=p_organization_id and v.id=p_template_version_id; select * into t from public.reporting_family_templates where organization_id=p_organization_id and id=v_template_id and archived_at is null; if not found or d.id is null or t.obligation_type<>(select obligation_type from public.reporting_obligations where organization_id=p_organization_id and id=d.obligation_id) or t.stage_kind<>(select stage_kind from public.reporting_obligation_stages where organization_id=p_organization_id and id=d.stage_id) then return query select 'not_found',null::jsonb;return;end if;
 ...
 if jsonb_typeof(v->'fields')<>'array' or exists(select 1 from jsonb_array_elements(v->'fields') field where field->'value'->>'type'='member_states') then return query select 'invalid_request',null::jsonb;return;end if;
...
returns table(outcome text,result jsonb) language plpgsql security definer set search_path = public, pg_temp as $$ declare t public.reporting_family_templates%rowtype; v_digest text; v_existing record; v_result jsonb; begin
 if p_idempotency_key is null or jsonb_typeof(p_content)<>'object' or not public.m5_triage_actor_has_permission(p_organization_id,p_actor_user_id,'can_edit_organization') then return query select 'invalid_request',null::jsonb;return;end if;
...
revoke all on function public.m6_reporting_stage_field_definitions(text,text),public.m6_reporting_draft_fields(jsonb,jsonb,jsonb,timestamptz),public.m6_reporting_draft_complete(jsonb,jsonb,jsonb),public.m6_reporting_family_template_json(uuid,uuid),public.m6_reporting_submission_json(uuid,uuid) from public,anon,authenticated;
revoke all on function public.create_reporting_family_template_atomic(uuid,uuid,text,text,text,text,jsonb,uuid,uuid) from public,anon,authenticated;
