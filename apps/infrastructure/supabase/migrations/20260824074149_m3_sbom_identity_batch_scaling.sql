-- Identity writes must be bounded by the current component batch. Re-scanning a
-- completed prefix for every batch makes 50,000-component documents quadratic.
...
begin
  ...
  from public.sbom_components c
  join jsonb_to_recordset(p_components) as x(document_local_ref text) on x.document_local_ref=c.document_local_ref
  ...
  from public.sbom_components c
  join jsonb_to_recordset(p_components) as x(document_local_ref text) on x.document_local_ref=c.document_local_ref
  where c.organization_id=p_organization_id and c.document_id=p_document_id and c.original_purl is not null
  ...
  from public.sbom_components c
  join jsonb_to_recordset(p_components) as x(document_local_ref text) on x.document_local_ref=c.document_local_ref
  where c.organization_id=p_organization_id and c.document_id=p_document_id and c.cpe is not null
  on conflict (organization_id,document_id,component_id,identity_type,original_value) do nothing;
