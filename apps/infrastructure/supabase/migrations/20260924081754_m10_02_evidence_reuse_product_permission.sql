-- Require product-view permission for M10 reverse links exposed through M8.
-- The base evidence reuse read retains its established evidence permission.
...
begin
  ...
  return query select 'found',jsonb_build_object(
    ...
    'frameworkControls',case when public.m10_actor_has_framework_permission(p_organization_id,p_actor_user_id,'can_view_frameworks')
      and public.m9_supplier_actor_can(p_organization_id,p_actor_user_id,'can_view_products') then coalesce((
    ...
    'frameworkControlsHasMore',case when public.m10_actor_has_framework_permission(
      ...
      and public.m9_supplier_actor_can(p_organization_id,p_actor_user_id,'can_view_products') then exists(
