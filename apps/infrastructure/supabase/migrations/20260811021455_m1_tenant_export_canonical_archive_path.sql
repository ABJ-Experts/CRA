-- A verified export must point only to the deterministic private archive that
-- the worker writes and hashes. An organization-prefix match could otherwise
-- mark a different object as downloadable.
...
begin
  ...
  if p_manifest_sha256 !~ '^[0-9a-f]{64}$' or p_artifact_sha256 !~ '^[0-9a-f]{64}$'
     ...
     or (
       p_artifact_object_path is not null
       and p_artifact_object_path <> (
