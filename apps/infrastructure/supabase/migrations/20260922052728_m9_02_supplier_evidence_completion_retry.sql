-- A reserve operation and a completion operation have distinct idempotency
-- scopes. The version is already bound to the scoped invitation and request
-- item at reservation time; completion must therefore not compare its key to
-- the reservation key before delegating integrity/idempotency to M8.
